...
#include "a2q9_sortCheck.h" /*We will use this file to determine sortedness of input.*/
...
void merge(const int A[], int lenA, const int B[], int lenB, enum sortedness direction, int C[])
{
    ...
    i = 0;
    j = 0;
    k = 0; /*Tops of stacks A, B and C, 0-based indexes*/
    while (i < lenA && j < lenB)
    { /*While the input stacks have cards*/
        /*Compare according to direction.*/
        if ((direction == ASCENDING) ? A[i] <= B[j] : A[i] >= B[j])
        {
            C[k] = A[i];
            i++;
        } /*Put top of A on C*/
    ...
    while (i < lenA)
    {
        ...
        i++;
        k++;
    } /*Append A to C if A has elements.*/
    while (j < lenB)
    {
        C[k] = B[j];
        j++;
...
void main(void)
{
    ...
    scanf("%d", &lenA);
    printf("Enter a sorted array of %d integers: ", lenA);
    input_array(A, lenA);
    sA = check_sorted(A, lenA);
    if (sA == UNSORTED)
    {
        printf("First array is unsorted. Sort first array in which direction? [A=Ascending/D=Descending] ");
        scanf("%s", choice);
        sA = (choice[0] == 'a' || choice[0] == 'A') ? ASCENDING : DESCENDING;
        printf("Sorting first array...\n");
        sort(A, lenA, sA);
    }
    printf("\nEnter length of second sorted array: ");
    scanf("%d", &lenB);
    printf("Enter another sorted array of %d integers: ", lenB);
    input_array(B, lenB);
    sB = check_sorted(B, lenB);
    if (sB == UNSORTED)
    {
        printf("Second sorted array is unsorted. Sort second array in which direction? [A=Ascending/D=Descending] ");
        scanf("%s", choice);
        sB = (choice[0] == 'a' || choice[0] == 'A') ? ASCENDING : DESCENDING;
        printf("Sorting second array...\n");
        sort(B, lenB, sB);
    }
    printf("\nMerge in which direction? [A=Ascending/D=Descending] ");
    scanf("%s", choice);
    sC = (choice[0] == 'a' || choice[0] == 'A') ? ASCENDING : DESCENDING;
    if (sA != sC)
    {
        printf("Reversing first array...\n");
        reverse_array(A, lenA);
        sA = sC;
    }
    if (sB != sC)
    {
        printf("Reversing second array...\n");

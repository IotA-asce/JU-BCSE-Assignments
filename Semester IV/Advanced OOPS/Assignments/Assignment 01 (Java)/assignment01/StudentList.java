...
public class StudentList {
    ...
    private ArrayList<Department> failedStudentsList; // list of failed students which is to be filled up at the end of
                                                      // the SetScore() method
     ...
     * 
     * deptStudentCount can increase and decrease as it is the number of students
     * present in a department deptStudentChronology can only increase as it
     * maintains roll count for students to take new admission
     */
    ...
    public void NewStudentEntry() // new entry is treated as new Admission
    {
        ...
        temporaryDept.admissionDate = calendar.get(Calendar.DATE) + "." + (calendar.get(Calendar.MONTH) + 1) + "."
                + calendar.get(Calendar.YEAR);
        temporaryDept.studentID = temporaryDept.department.toUpperCase(Locale.ENGLISH) + year + buff
                + deptStudentChronology.get(index);
    ...
    void PrepareMarksheet() {
        ...
        if (index != -1) {
            ...
            System.out.println(
                    "_______________________________________________________________________________________________________");
            System.out.println("Name : " + departments.get(index).studentName + "\t\t\tRoll no : "
                    + departments.get(index).studentID + "\t\t\tAdmission date : "
                    + departments.get(index).admissionDate);
            System.out.println(
                    "_______________________________________________________________________________________________________");
            ...
            for (int k = 0; k < 5; k++) {
                ...
                System.out.println("\tSubject " + (k + 1) + "\t\t\t\t" + departments.get(index).score[k] + "\t\t\t\t\t"
                        + pass_fail);
                System.out.println(
                        "_______________________________________________________________________________________________________");
                pass_fail = null;
            ...
            // {
            // System.out.println("Score for this student is not set, unable to produce
            // marksheet");
            // return;
            // }
            // System.out.println("\n\n________________________________________________________________________________");
            // System.out.println("________________________________________________________________________________");
            // System.out.println("Name : " + failedStudentsList.get(index).studentName +
            // "\t\tRoll no : " + failedStudentsList.get(index).studentID + "\tAdmission
            // date : " + failedStudentsList.get(index).admissionDate);
            // System.out.println("________________________________________________________________________________");
            ...
            // failedStudentsList.get(index).score[k] + "\t\t\t\t\t" + pass_fail);
            // System.out.println("________________________________________________________________________________");
            // pass_fail = null;
            // }
    ...
    public void ShowSortedListOfStudentsFromADepartment() {
        /*
         * Operations to perform 1. check for the existence of the department from the
         * deptName mark index and display count 2. Search the whole Departments for the
         * students with matching departments 3. Flush the students into the buffer
         * arraylist (better to insert the students based on total score) 4. loop
         * through the arraylist buffer and display the students available
         */
        String tdep = "";
        int index = -1;
        int checkTotal = 0;
        ...
        System.out.println(
                "|   To get the sorted list of the students of a particular department type in the necessary details    |");
        System.out.println(
                "________________________________________________________________________________________________________");
        ...
        if (index != -1) {
            System.out.println("--------> No of student in " + tdep.toUpperCase(Locale.ENGLISH) + " : "
                    + deptStudentCount.get(index));

            /*
             * Operation 1 confirmed here Operation 2 to commence
             */
            for (int k = 0; k < departments.size(); k++) {
                ...
                if (departments.get(k).department.equalsIgnoreCase(tdep)) {
                    /*
                     * if it matches then the total is to be checked if total is greater than
                     * checkTotal it is inserted before the given index
                     */
                    int cot = buffer.size();
                    if (cot == 0) {
                        cot = 1;
